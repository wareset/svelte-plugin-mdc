...
export const F = (MDC) => MDC.getDefaultFoundation();
export const FA = (MDC_or_F) => MDC_or_F.adapter || F(MDC_or_F).adapter;

export function fixes_on_init(self) {
  const root = self.root || self.root_;
  if ('id' in self && !self.id) self.id = uid();

  // Fix if MDC is MDCList
  (() => {
    if (!(self instanceof MDC.MDCList)) return;
    // listbox, radiogroup, group
    let role = root.getAttribute('role');
    if (!role) root.setAttribute('role', (role = 'listbox'));
    const role_child = {
      listbox: 'option',
      radiogroup: 'radio',
      group: 'checkbox',
    ...
    const sc = 'mdc-list-item--selected';
    self.listElements.forEach((node, k) => {
      if (node.getAttribute('role') !== role_child) {
        node.setAttribute('role', role_child);
      }

      if (!k || node.classList.contains(sc)) {
        node.setAttribute('tabindex', 0), F(self).focusItemAtIndex(k);
      }

      const gc = node.querySelector(`.mdc-list-item__graphic-controls`);
      if (!gc) return;
      const input = gc.querySelector(`.mdc-${role_child}`);

      [...gc.children].forEach((n) => {
        if (n === input) return;
        try {
          if (TARGETS.has(n)) get(TARGETS.get(n)).destroy();
        } catch (err) {}
...
export function fixes(self) {
  ...
  (() => {
    if (!(self instanceof MDC.MDCList)) return;
    // try {
    // if (!(self.selectedIndex >= 0)) F(self).focusFirstElement(0);
    // } catch (err) {}
  })();
}
